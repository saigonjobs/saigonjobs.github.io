$(document).ready(function() {
   ...
   $(social_add_button).click(function(e) {
    ...
    // Job Tags
    var tag_wrapper =$(".job_tags_section");
    var tag_add_button =$(".add_job_tag");
    var d_tag = '<div><div id="job_tag" class="form-inline"><input type="text" class="form-control" id="job_tags" name="job_tags" pattern="[/^\\S+$/]+" required placeholder="Tag"> <button class="tags_remove btn btn-danger">-</button> </div>';
    $(tag_add_button).click(function(e) {
        e.preventDefault();        
        $(tag_wrapper).append(d_tag); //add input boxes        
    });
    
    $(tag_wrapper).on("click", ".tags_remove", function(e) {
        e.preventDefault();
        $(this).parent('div').remove();
    });
